...
void apply_kernelsu_rules()
{
	...
	pr_info("KSU_TRACE: APPLY_RULES ENTER pid=%d comm=%s jiffies=%lu\n",
...
out_unlock:
	...
	pr_info("KSU_TRACE: POLICY_LOCK BEFORE pid=%d comm=%s jiffies=%lu\n",
		current->pid, current->comm, jiffies);
	write_lock(lock);
	pr_info("KSU_TRACE: POLICY_LOCK AFTER pid=%d comm=%s jiffies=%lu\n",
		current->pid, current->comm, jiffies);
	preempt_enable();
	...
	pr_info("KSU_TRACE: POLICY_APPLY BEFORE pid=%d comm=%s jiffies=%lu\n",
		current->pid, current->comm, jiffies);
	apply_kernelsu_rules_fn((void *)db);
	pr_info("KSU_TRACE: POLICY_APPLY AFTER pid=%d comm=%s jiffies=%lu\n",
		current->pid, current->comm, jiffies);
	goto out_unlock;
...
out_unlock:
	preempt_disable();
	pr_info("KSU_TRACE: POLICY_UNLOCK pid=%d comm=%s jiffies=%lu\n",
		current->pid, current->comm, jiffies);
	write_unlock(lock);
...
out_flush:
	...
	reset_avc_cache();
#endif
	pr_info("KSU_TRACE: APPLY_RULES EXIT pid=%d comm=%s jiffies=%lu\n",
		current->pid, current->comm, jiffies);
}
